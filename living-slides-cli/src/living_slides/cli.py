...
@click.option("--from", "from_format", default=None,
              type=click.Choice(["pptx"]),
              help="Convert from another format (currently: pptx)")
@click.option("--out", "out_path", default=None,
              help="Output HTML path when converting from another format")
def adopt(filepath: str, strategy: str, dry_run: bool,
          from_format: str | None, out_path: str | None):
    ...
    With --from pptx, converts a PowerPoint file into a slive HTML deck
    (requires the optional 'pptx' extra: pip install 'living-slides[pptx]').
    """
    if from_format == "pptx":
        from living_slides.adopters.pptx import adopt_pptx
        try:
            generated = adopt_pptx(filepath, output_html=out_path)
        except RuntimeError as e:
            raise click.UsageError(str(e))
        except FileNotFoundError as e:
            raise click.UsageError(str(e))
        click.echo(f"Imported PPTX → {generated}")
        return
